...
"""
str1="19.4.305"
str2="Comparison of Org 25969 in Elderly Subjects With Adult Subjects (19.4.305)(P05950)"
...
list1 = ['P06042','P101','19.4.306','19.4.334','P07981','P105','19.4.301','19.4.302','19.4.310','19.4.303','P07982','19.4.110','19.4.112','19.4.113','19.4.114','19.4.115','19.4.116','19.4.313','19.4.316','19.4.318','19.4.324','P06101','19.4.328','19.4.333','19.4.335','P07025','P07044','P07038','19.4.319','19.4.101','19.4.102','19.4.105','19.4.106','19.4.107','19.4.108','19.4.109','19.4.201','19.4.202','19.4.203','19.4.204','19.4.205','19.4.206','19.4.207','19.4.208A','19.4.208B','19.4.209A','19.4.209B','19.4.210','19.4.304','19.4.305','19.4.308','19.4.309','19.4.311','19.4.312']
...
OrgIDFirst=sum(FDAOrg1, [])  ## Check for duplicates
...
NCTFirst=sum(alist, []) ## Check for duplicates
print(NCTFirst)
"""set = set(final)
result = list(set)
"""
...
"""
OrgIDSecond=sum(OrgIDFirst, [])
final2=sum(alist2, [])



#NCTsecond=sum(NCTFirst, [])

print(OrgIDSecond)

print(len(OrgIDSecond))
print(len(NCTFirst))
"""
#NCTfinal=sum(NCTFirst, [])


#

# Now merging the two lists to create a list of unique NCTs
"""
for word1 in final_additional:
    if word1 not in NewNCT:
        NewNCT.append(word1)
"""
#

OrgIDSecondFinal=sum(OrgIDSecond, [])
NCTSecondFinal=sum(NCTSecond, [])


FinalOrg = OrgIDFirst+OrgIDSecondFinal

FinalNCT = NCTFirst + NCTSecondFinal
...
#print("Printng here"+ NCTFirst)

for indic11 in FinalNCT:
    ...
    #for i in range(0,len1):
     ...
     for item in xml.findAll('clinical_study'):
         ...
         if party is None:
             ...
             NewItem = party.text
             NewDate = parse_date(NewItem)
             if NewDate < datetime.datetime(2015, 4, 4):  # YYYY-MM-DD
                 SDate.append(NewDate)
             else:
                 SDate.append("empty")
...
try:
    ...
    #for i1 in list11:
    for i1,i111,i2,i3,i4,i5,i6,i7,i8,i9,i10,i11,i12,i13,i14,i15,i16,i17,i18,i19,i20, i21, i22, i23,i24, i25, i26, i27 in zip(FinalOrg,OrgID, NCTID, Btitle,Condition, OverallStatus,Intervention, acronym, agency, gender, MAge, Phase1, AgencyClass, StudyType, StudyDesign, SecID, FirstRecv, SDate, ComplDate, LastChangedDate, VerificationDate, FirstRecvResultDate, FirstRecvResultDispoDate, PCD, Countries, Rm_Countries, IsFDA,Is801):
...
for row in allTrials:
    if ((name1.lower() in str(row[6]).lower() or name2.lower() in str(row[6]).lower() or name3.lower() in str(row[8]).lower() or name4.lower() in str(row[8]).lower()) and name5.lower() not in str(row[17].lower())):
...
for row in registered_trials:
    ...
    d = seq.ratio() * 100
    seq1 = difflib.SequenceMatcher(None, str(row[0]).lower(), str(row[15]).lower())
    d1 = seq1.ratio() * 100
    seq2 = difflib.SequenceMatcher(None, str(row[0]).lower(), str(row[3]).lower())
    d2 = seq2.ratio() * 100

    if (d>59 or d1>59 or d2>95):
        matchedrows.append(row)

matchedrows1=[]
for i in list1:
    for row in registered_trials:
        if i.lower() in str(row[0]).lower():
            matchedrows1.append(row)


# str(row[0]).lower() in str(row[3]).lower()
## Matching finished
