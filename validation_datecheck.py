...
for i in FirstRecv:
    ...
    if y3 is not None:
     ...
     y2=re.search('([a-zA-Z]*)',i)
     if y2:
      year=y2.group(1)
     #print(month)

      year1=(dict.get(month_dict, year, "default"))
      print (year1)
      day=y2.group(1)
      print(day)
      final=month1+"/"+year1+"/"+day2
      print(final)
      datelist.append(str(final))
...
try:
    ...
    #for i1 in list11:
    for i1,i111,i2,i3,i4,i5,i6,i7,i8,i9,i10,i11,i12,i13,i14,i15,i16,i17,i18,i19,i20, i21, i22, i23,i24, i25, i26, i27 in zip(final3,OrgID, NCTID, Btitle,Condition, OverallStatus,Intervention, acronym, agency, gender, MAge, Phase1, AgencyClass, StudyType, StudyDesign, SecID, datelist, SDate, ComplDate, LastChangedDate, VerificationDate, FirstRecvResultDate, FirstRecvResultDispoDate, PCD, Countries, Rm_Countries, IsFDA,Is801):
...
for row in allTrials:
    registration_pool.append(row)
'''
for row in registration_pool:
    row[16]=row[16].strftime('%m/%d/%Y')
'''
#datelist = datelist.strftime('%m/%d/%Y')
# for row in allTrials:
#     print row[0]
...
for row in registration_pool:
    if((row[16] != "")):
        reportingDateMatch = re.search(r'(\d+)[/-](\d+)[/-](\d+)', row[16], re.M|re.I)
        if(reportingDateMatch):
            reportingDateMonth = reportingDateMatch.group(2)
            reportingDateYear = str(reportingDateMatch.group(1))
            reportingDateDay= str(reportingDateMatch.group(3))
        ...
        #print "report date is " + str(trialReportingDate)
            ...
            if(trialReportingDate<datetime.date(2013, 4, 1)):
             reported_trials.append(row)
            else:
             print ('trial  ' + str(row[0]) + " is not timely reported, so excluded")



print ("")
#print "Reported trials result is "+str(len(reported_trials)) + "/" + str(len(reporting_pool)) + " = " + str((Decimal(len(reported_trials))/Decimal(len(reporting_pool)))*100)+"%"
...
for row in reported_trials:
    if (name1.lower() in str(row[6]).lower() or name2.lower() in str(row[6]).lower() or name3.lower() in str(row[8]).lower() or name4.lower() in str(row[8]).lower()):
        finals.append(row)
print ("number of valid trials "+str(len(finals)))
for row in finals:
    print (row[2])
