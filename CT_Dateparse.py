 ...
 for i in range(1,total_pages+1):
    ...
    for child in page_root:
        if child.tag == 'clinical_study':
            ...
            NewNCT.append(nct_id)
            nct_data = get_nct_data(nct_id)
            #print(nct_data)

print("This is new NCT: "+str(NewNCT))  # All the NCTs now obtained.
list3= str(NewNCT)


# Now checking for inconsistent IDs with Drug Name combination
    ...
    #for i in range(0,len1):
     ...
     for item in xml.findAll('clinical_study'):
      ...
      if party is None:
       ...
       if NewDate < datetime.datetime(2015, 9, 25): # YYYY-MM-DD
...
for row in allTrials:
    if ((name1.lower() in str(row[5]).lower() or name2.lower() in str(row[5]).lower() or name3.lower() in str(row[7]).lower() or name4.lower() in str(row[7]).lower()) and name5.lower() not in str(row[16].lower())):
