...
if (!isset($_SESSION["is_auth"])) {
	...
	if(isset($_POST['select'])){
		...
		switch ($select) {
			...
			case 'all':
				$sql = "SELECT FDA_Drug_Name,CT_NCT,CT_Conditions,CT_Phase,CT_Enrollment FROM master";
				$records = mysql_query($sql);
			break;
			

            case 'DrugName':
				$sql = "SELECT FDA_Drug_Name,CT_NCT,CT_Conditions,CT_Phase,CT_Enrollment FROM master where FDA_Drug_Name LIKE '%$search%'";
				$records = mysql_query($sql);
			break;
...
<body>
	...
	<h1 align=center>Bioethics International Database : Guest Page</h1>
	<div align = "center">	
